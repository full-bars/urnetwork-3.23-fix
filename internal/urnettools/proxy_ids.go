...
// loadClientJWTStoreFromPath reads a client JWT store from an explicit file
// path. Used for testing and for the case where the caller already knows
// the exact location (e.g. the global ~/.urnetwork/.client_jwts.json).
func loadClientJWTStoreFromPath(path string) (map[string]clientJWTEntryMinimal, error) {
	data, err := os.ReadFile(path)
	if err != nil {
		if os.IsNotExist(err) {
			return map[string]clientJWTEntryMinimal{}, nil
...
// cmdProxyIDs reads the client JWT store for the targeted provider and prints
// each proxy's client_id. This is a read-only, single-target subcommand.
func cmdProxyIDs(args []string) error {
	all, include, exclude, t, positionals, err := parseProxyBatchFlags(args)
	if err != nil {
		return err
	}
	if all || len(include) > 0 || len(exclude) > 0 {
		return fmt.Errorf("proxy ids operates on ONE provider — use --unit/--user/--network to target it")
	}
	providers := Discover()
	if len(positionals) > 0 {
		// Accept target flags passed as positionals.
		t2, _, err2 := parseTargetFlagsLenient(positionals)
		if err2 == nil {
			t = t2
		}
	}
...
// parseProxyBatchFlags extracts --all/--include/--exclude flags and returns
// the remaining args for target parsing. Returns batch selections, the parsed
// target, and any leftover positionals.
func parseProxyBatchFlags(args []string) (all bool, include, exclude []string, t Target, positionals []string, err error) {
	var rest []string
