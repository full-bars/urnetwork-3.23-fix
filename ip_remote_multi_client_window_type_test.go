...
func TestPerformanceProfileFixedWindow(t *testing.T) {
	...
	assert.Equal(t, WindowTypeSpeed, windowType)
	assert.Equal(t, DefaultWindowSizeSettings(), windowSize)
}
...
func TestMultiClientSelectWindowTypesAuto(t *testing.T) {
	...
	assert.Equal(t, []WindowType{WindowTypeQuality, WindowTypeSpeed}, multiClient.selectWindowTypes(webPacket))
	assert.Equal(t, []WindowType{WindowTypeSpeed, WindowTypeQuality}, multiClient.selectWindowTypes(otherPacket))
	...
	assert.Equal(t, []WindowType{WindowTypeSpeed}, multiClient.selectWindowTypes(webPacket))
	assert.Equal(t, []WindowType{WindowTypeSpeed}, multiClient.selectWindowTypes(otherPacket))
}
...
func TestMultiClientAutoWindowSizeIgnored(t *testing.T) {
	...
	paths, _ := multiClient.affinityIpPathsWithLock(ipPath)
	assert.Equal(t, 1, len(paths))
	assert.Equal(t, &IpPath{Version: 4}, paths[0])
...
func TestMultiClientNetworkAllowDirectAuto(t *testing.T) {
	...
	assert.Equal(t, true, pp != nil)
	assert.Equal(t, true, pp.AllowDirect)
	assert.Equal(t, WindowTypeAuto, pp.WindowType)
