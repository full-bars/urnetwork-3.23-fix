...
### Changed
- **Adaptive GC consolidation (PR #428)**: the separate eco memory monitor runtime loop was removed from `provider/main.go` and its host available RAM signal folded into a single consolidated adaptive GC governor in the pressure monitor (`resource_pressure.go`, `gcGovernor` / `runPressureMonitor`). The governor is now the single writer to the Go GC percentage knob and applies to all profiles (baseline no-profile, auto Tier 1-4, turbo, eco). It only ever lowers GOGC below the profile baseline, taking the tighter of process heap fraction and host available RAM. A 10s live-heap subtick reacts to heap spikes faster than the 30s sweep. New `URNETWORK_ADAPTIVE_GC` kill switch (`0`/`false`/`off`/`no` disables; on by default); if the operator sets `GOGC` the governor backs off entirely. The pressure status file now reports `gc_state` and `heap_frac`. The eco profile still applies its static startup baseline (GOGC 50, 75% RAM GOMEMLIMIT); only the separate runtime eco-monitor loop is retired. The `[eco]` memory pressure log lines are replaced by `[proxy][pressure] gcGovernor` lines.
