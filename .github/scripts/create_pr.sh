...
EXISTING_PR=$(gh pr list --head "$BRANCH_NAME" --state open --json number,url --jq '.[0] | "\(.number) \(.url)" // empty')
...
if [ -n "$EXISTING_PR" ]; then
  ...
  PR_URL=$(gh pr create --title "$PR_TITLE" --body-file "$PR_BODY_FILE" --label "security" --json url --jq '.url')
