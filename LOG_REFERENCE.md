...
A source is labeled by host and path only. The query string and any credentials in the URL are never logged in these lines, since source URLs often carry an API token. A path segment that looks like a credential (the segment after `/token/` or `/key/`, or a long opaque blob) is shown as `[redacted]`, including when the secret contains an encoded slash (`%2F`). A URL that cannot be parsed is shown by its host alone. Two sources that would share a label are numbered (`#2`).
