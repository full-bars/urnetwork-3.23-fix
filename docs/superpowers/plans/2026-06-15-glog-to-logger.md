...
**Date:** 2026-06-15
**Repo:** `/home/user/ur/urnetwork-3.23-fix`
**Branch:** `refactor/glog-to-logger` (new branch off `master`)
**Upstream reference (already refactored):** `/home/user/ur/connect`
...
conflict surface. The pattern is mechanical and already proven in `/home/user/ur/connect`.

## Mandatory workflow constraints (read first)

> [!IMPORTANT]
> **All work goes through a new branch and a PR.** Do not commit to `master`.
> 1. `cd /home/user/ur/urnetwork-3.23-fix && git checkout master && git checkout -b refactor/glog-to-logger`
...
```bash
cp /home/user/ur/connect/log.go /home/user/ur/urnetwork-3.23-fix/log.go
```
...
> upstream formatting. After `cp`, run `diff log.go /home/user/ur/connect/log.go` —
...
`/home/user/ur/connect/transfer_encrypt.go` (they match the fork structurally).
...
- Top-level constructor (e.g. `NewLocalUserNat*`) resolves from its settings:
  ...
  standard doc comment). Cross-check field name against `/home/user/ur/connect/ip.go`.
...
3. Diff your constructor signatures and struct field placements against
   `/home/user/ur/connect/ip.go` to confirm parity.
...
1. Add `Log Logger` to `WebRtcSettings` if not present (cross-check
   `/home/user/ur/connect/transport_p2p_webrtc.go`).
...
- In `net.go`, the standalone resolution `if log := loggerOrDefault(self.Log).V(2); log.Enabled() {`
  ...
  receiver). Cross-check `/home/user/ur/connect/net.go` lines around the 3 calls.
...
Receiver methods → `self.log.X`. Cross-check `/home/user/ur/connect/message_framer.go`.
...
`/home/user/ur/connect/tuning.go`.
...
```bash
cd /home/user/ur/urnetwork-3.23-fix
...
> /home/user/ur/connect/<file>`). If upstream keeps it, leave it. If upstream removed
...
- `/home/user/ur/connect/log.go` — the shim (verbatim source for Task 1)
- `/home/user/ur/connect/transfer.go` — Client hierarchy end-state
- `/home/user/ur/connect/transfer_contract_manager.go` — `contractQueue` log field + `client.log` pattern
- `/home/user/ur/connect/transfer_route_manager.go` — `NewRouteManagerWithLogger` entry point
- `/home/user/ur/connect/transfer_encrypt.go` — `log Logger` helper params
- `/home/user/ur/connect/transport.go`, `transport_pt.go`, `transport_p2p_webrtc.go`
- `/home/user/ur/connect/ip.go`, `ip_remote_multi_client.go`
- `/home/user/ur/connect/net.go`, `net_http.go`, `message_framer.go`, `message_pool.go`, `trace.go`
