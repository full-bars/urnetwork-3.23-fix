...
| `URNETWORK_PPROF` | - | Set to a `host:port` to enable the Go pprof profiler on a loopback-only listener (e.g. `127.0.0.1:6060`). Off by default. Serves `/debug/pprof/*`; only literal loopback IPs are accepted (hostnames are rejected). Pull profiles via an SSH tunnel, e.g. `ssh -L 6060:127.0.0.1:6060 host` then `go tool pprof http://127.0.0.1:6060/debug/pprof/profile`. |
