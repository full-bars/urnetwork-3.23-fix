...
| `PROXY_URL` *(planned)* | - | Live proxy list URL, fetched and merged on an interval. Comma-separate for multiple sources. See [Proxy URL Sources](Proxy-URL-Sources.md). |
| `PROXY_URL_REFRESH` *(planned)* | `15m` | How often `PROXY_URL` is re-fetched to add new entries. |
| `PROXY_URL_MAX` *(planned)* | unlimited | Caps total proxies sourced from `PROXY_URL`. |
| `PROXY_DEAD_CLEANUP_SCOPE` *(planned)* | `none` | `none`, `url`, or `all` — which sources the automatic daily dead-proxy cleanup may touch. |
| `PROXY_DEAD_CLEANUP_INTERVAL` *(planned)* | `24h` | Cadence of the automatic cleanup job, when scope isn't `none`. |
