...
func TestPeerIdentitiesEstablishedAndVerified(t *testing.T) {
	...
	assert.Equal(t, 0, len(a.EncryptionSessionManager().PeerIdentities()))
	assert.Equal(t, 0, len(b.EncryptionSessionManager().PeerIdentities()))
	...
	assert.Equal(t, true, 0 < atomic.LoadInt64(&aChanges))
	assert.Equal(t, true, 0 < atomic.LoadInt64(&bChanges))
}
